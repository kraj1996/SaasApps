...
const TestSelectModalForDaySpecial = ({ show, handleClose, DiscountData }) => {
  ...
  const getDiscountMasterItemData = () => {
    axios
      .post("/api/v1/Promotional/getPromotionalMasterItemData", {
  ...
  const disableData = (DiscountID, InvestigationId) => {
    if (window.confirm("Are You Sure?")) {
      axios
        .post("/api/v1/Promotional/RemoveInvestigationPromotionalMaster", {
  ...
  const Api = () => {
    ...
    if (data.length > 0) {
      ...
      axios
        .post("/api/v1/Promotional/AddPromotionalMasterItem", payload)
        .then((res) => {
          if (res.data.message) {
            toast.success(res.data.message);
            const val = InvestigationData.map((ele) => {
